...
private:
    ...
    void selectedArea(QPixmap selectedPixmap);
    CustomScene* scene;
    QGraphicsView* view;
...
void ImageViewer::selectedArea(QPixmap selectedPixmap) {
    if (!pixmap.isNull()) {
...
void ImageViewer::moreInfoAboutImage() {
    if (loadFile) {
        ...
        if (selected) {
            scene->getPixmap().save("selected.bmp");
            BMPImageReader f;
            selectedImgAndPixels = f.loadBMPFIle(QString("selected.bmp"));

            //selectedImage = *(selectedImgAndPixels->img);
            numberСolors(mathLoadedImage, selectedImgAndPixels);
            histogram = new Histogram(mathLoadedImage, this);
            QFile::remove("selected.bmp");
        }
        else {
            numberСolors(mathSelectedImage, loadedImgAndPixels);
...
{
    if (loadFile) {
        // установка изначального разрешения файла
        view->resetTransform();
    }
}
...
void ImageViewer::open() {
    QString fileName = QFileDialog::getOpenFileName(this, "Open Image", "", "Images (*.bmp)");

    if (!fileName.isEmpty()) {
        BMPImageReader d;
        loadedImgAndPixels = d.loadBMPFIle(fileName);
...
void ImageViewer::numberСolors(MathImage* mathImage, Returned* imgAndPixels) {
    ...
    if (loadFile == true) {
        if ((fileName != fileNameChek) || (fileNameChek == NULL)) {
